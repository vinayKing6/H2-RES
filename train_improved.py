"""
DDPG Training Script for H2-RES System (Improved Version)
Version: V8.1 - 训练稳定性改进版
改进：噪声衰减 + 批次增大 + TAU调整 + 确定性评估
"""
...
# --- Hyperparameters (改进版) ---
MAX_EPISODES = 20000  # 完整训练20000轮
MAX_STEPS = 24  # 24 hours per episode
BATCH_SIZE = 256  # ✨ 改进1：从16增加到256，大幅减少梯度方差，提升训练稳定性
GAMMA = 0.99
TAU = 0.005  # ✨ 改进2：从0.001增加到0.005，加快目标网络更新
LR_ACTOR = 1e-4
LR_CRITIC = 1e-3
REPLAY_SIZE = 1000
...
class NoiseScheduler:
    ...
    Args:
        initial_noise: 初始噪声水平（默认0.2）
        final_noise: 最终噪声水平（默认0.02）
        decay_episodes: 衰减周期（默认15000轮）
    """
    def __init__(self, initial_noise=0.2, final_noise=0.02, decay_episodes=15000):
...
def evaluate_policy(env, agent, eval_episodes=4):
    ...
    Args:
        env: 环境实例
        agent: DDPG智能体
        eval_episodes: 评估轮数（默认4轮，对应春夏秋冬）
...
def train():
    ...
    # ✨ 改进：使用新的TAU值，并传入device="cuda"强制GPU训练
    agent = DDPGAgent(
        state_dim=state_dim,
    ...
    # ✨ 改进：创建噪声调度器
    noise_scheduler = NoiseScheduler(initial_noise=0.2, final_noise=0.02, decay_episodes=15000)
    ...
    rewards_history = []
    h2_prod_history = []
    eval_rewards_history = []  # ✨ 新增：确定性评估奖励
    eval_h2_history = []  # ✨ 新增：确定性评估制氢量
    noise_history = []  # ✨ 新增：噪声历史
    
    print("\n" + "=" * 60)
    print(f"  开始训练 DDPG V8.1 模型 - 共 {MAX_EPISODES} 轮")
    print("=" * 60)
    print(f"环境版本: V10 (真正的Gemini实现 - 修复功率分配逻辑)")
    print(f"训练改进: V8.1 (噪声衰减 + 批次增大 + TAU调整 + 确定性评估 + 强制GPU)")
    print(f"观测空间: {state_dim}维 (包含历史+时间+燃料电池状态)")
    print(f"数据长度: {len(df_data)} 小时")
    print(f"每轮步数: {MAX_STEPS} 小时")
    print(f"Batch Size: {BATCH_SIZE} (改进: 16→256，提升16倍)")
    print(f"TAU: {TAU} (改进: 0.001→0.005)")
    print(f"噪声衰减: 0.2→0.02 (15000轮)")
    ...
    for episode in range(MAX_EPISODES):
        ...
        # ✨ 改进：每100轮进行确定性评估
        if (episode + 1) % 100 == 0:
            eval_reward, eval_h2 = evaluate_policy(env, agent, eval_episodes=4)
            ...
            print("-" * 60)
            print(f"[统计] 最近100轮训练平均: Reward={avg_reward_100:.2f}, H2={avg_h2_100:.2f} kg")
            print(f"[评估] 确定性策略性能: Reward={eval_reward:.2f}, H2={eval_h2:.2f} kg")
            print("-" * 60)
        
        # 每1000轮保存检查点
        if (episode + 1) % 1000 == 0:
            checkpoint_path = f'results/ddpg_improved_ep{episode+1}.pth'
    ...
    agent.save('results/ddpg_improved.pth')
    np.save('results/training_rewards_improved.npy', rewards_history)
    np.save('results/training_h2prod_improved.npy', h2_prod_history)
    np.save('results/eval_rewards_improved.npy', eval_rewards_history)
    np.save('results/eval_h2_improved.npy', eval_h2_history)
    np.save('results/noise_history.npy', noise_history)
    ...
    print(f"\n模型已保存到: results/ddpg_improved.pth")
    print("=" * 60 + "\n")
