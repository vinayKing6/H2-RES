...
Version: V12.0 - 使用V12环境（Agent-Driven Allocation）
核心改进：
  1. 移除'负荷优先'硬约束
  2. Agent自由分配功率
  3. 环境只负责总功率约束
...
def train_dqn_v12(num_episodes=20000, save_freq=2000):
    """训练DQN模型（V12版本：使用V12环境）"""
    ...
    if not torch.cuda.is_available():
        ...
        print(f"[OK] 显存大小: {torch.cuda.get_device_properties(0).total_memory / 1024**3:.2f} GB")
    print("=" * 30 + "\n")
    ...
    # 关键参数设置
    MAX_STEPS = 24  # 每个episode强制只跑24小时
    
    print("\n" + "=" * 60)
    print(f"  开始训练 DQN V12 模型 - 共 {num_episodes} 轮")
    print("=" * 60)
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            episode_unmet += info.get('p_unmet', 0.0)  # V12新增：缺电统计
            steps += 1
            state = next_state
        ...
        if (episode + 1) % 100 == 0:
            ...
            print("-" * 80)
            print(f"[统计] 最近100轮平均: Reward={avg_reward_100:.2f}, "
                  f"H2={avg_h2_100:.2f} kg, Unmet={avg_unmet_100:.2f} kW")
            print("-" * 80)
        
        # 保存模型
        if (episode + 1) % save_freq == 0:
            agent.save(f'results/dqn_v12_ep{episode+1}.pth')
    ...
    agent.save('results/dqn_v12.pth')
    np.save('results/dqn_v12_training_rewards.npy', rewards_history)
    np.save('results/dqn_v12_training_h2prod.npy', h2_prod_history)
    np.save('results/dqn_v12_training_unmet.npy', unmet_history)
    ...
    print(f"  平均缺电: {final_avg_unmet:.2f} kW")
    print(f"  最终Epsilon: {agent.epsilon:.3f}")
    print(f"\n模型已保存到: results/dqn_v12.pth")
    print("=" * 60 + "\n")
...
if __name__ == '__main__':
    train_dqn_v12(num_episodes=20000, save_freq=2000)